...
from cpg_flow import targets, stage, workflow

from cpg_seqr_loader import jobs

from cpg_utils import Path, to_path, config, cloud
...
class CreateVdsFromGvcfsWithHailCombinerStage(stage.MultiCohortStage):
...
@stage.stage(
    required_stages=[CreateVdsFromGvcfsWithHailCombinerStage], analysis_type='matrixtable', analysis_keys=['mt']
)
class CreateDenseMtFromVdsWithHailStage(stage.MultiCohortStage):
    ...
    def queue_jobs(self, multicohort: targets.MultiCohort, inputs: stage.StageInput) -> stage.StageOutput:
        ...
        return self.make_outputs(multicohort, outputs, job)


@stage.stage(analysis_keys=['vcf'], required_stages=[CreateDenseMtFromVdsWithHailStage])
class ConcatenateVcfFragmentsWithGcloudStage(stage.MultiCohortStage):
    """
...
@stage.stage(required_stages=[ConcatenateVcfFragmentsWithGcloudStage])
class TrainVqsrIndelModelStage(stage.MultiCohortStage):
    ...
    def queue_jobs(self, multicohort: targets.MultiCohort, inputs: stage.StageInput) -> stage.StageOutput:
        ...
        composed_sitesonly_vcf = inputs.as_str(multicohort, ConcatenateVcfFragmentsWithGcloudStage)
...
@stage.stage(required_stages=[CreateDenseMtFromVdsWithHailStage, TrainVqsrSnpTranches])
...
class RunTrainedIndelVqsrOnCombinedVcf(stage.MultiCohortStage):
    """
    ...
    def queue_jobs(self, multicohort: targets.MultiCohort, inputs: stage.StageInput) -> stage.StageOutput:
        outputs = self.expected_outputs(multicohort)
        annotated_vcf = inputs.as_path(target=multicohort, stage=RunSnpVqsrOnFragmentsStage)
...
@stage.stage(analysis_type='custom', required_stages=[CreateDenseMtFromVdsWithHailStage])
class AnnotateFragmentedVcfWithVep(stage.MultiCohortStage):
    """
...
class AnnotateCohortSmallVariantsWithHailQuery(stage.MultiCohortStage):
    """
    ...
    def queue_jobs(self, multicohort: targets.MultiCohort, inputs: stage.StageInput) -> stage.StageOutput:
        ...
        variant_mt = inputs.as_path(target=multicohort, stage=CreateDenseMtFromVdsWithHailStage, key='mt')
...
@stage.stage(required_stages=[AnnotateCohortSmallVariantsWithHailQuery])
class SubsetMatrixTableToDatasetUsingHailQuery(stage.DatasetStage):
    """
...
class AnnotateDatasetSmallVariantsWithHailQuery(stage.DatasetStage):
