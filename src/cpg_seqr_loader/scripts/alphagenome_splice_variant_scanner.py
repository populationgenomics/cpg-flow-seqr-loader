...
def build_parser() -> argparse.ArgumentParser:
    ...
    P.add_argument("--merge-distance", dest="merge_distance", type=int, default=300,
                   help="Max bp gap to merge regions.")
    P.add_argument("--window-size", dest="window_size", type=int, default=100, help="Sliding window size (bp).")
    P.add_argument("--scan-span", dest="scan_span", type=int, default=50000,
                   help="Bp each side of variant center to scan.")
    P.add_argument("--plot-non-sig", dest="plot_non_sig", action="store_true", help="Plot even when not significant.")
    P.add_argument("--scan-all-tracks", dest="scan_all_tracks", action="store_true",
                   help="Scan all tracks (recommended).")
    P.add_argument("--epsilon", type=float, default=1e-8, help="Add to REF to avoid divide-by-zero.")
    P.add_argument("--output-table", default="alphagenome_scan_results.csv",
                   help="Output summary table path (csv/tsv/xlsx).")
    P.add_argument("--output-table-sum", default="alphagenome_scan_results_variant_organ_summary.csv",
...
def plot_variant_tracks(variant, interval, vout, transcript_extractor, plot_size: int, outpath: Path):
    ...
    if ref_rna:
        plot_elements.append(plot_components.OverlaidTracks(
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for ontology in organs:
        ...
        for _, row in variants_df.iterrows():
            ...
            # track names
            try:
                track_names = vout.reference.splice_sites.metadata.name + ': ' + vout.reference.splice_sites.metadata.strand
            except Exception:
            ...
            sig_any = any((r['is_significant'] and r['chrom'] == variant.chromosome and r['pos'] == variant.position and
                           r['ontology'] == ontology) for r in results_rows[-n_tracks:])
