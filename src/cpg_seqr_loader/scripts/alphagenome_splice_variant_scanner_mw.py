...
def build_parser() -> argparse.ArgumentParser:
    ...
    P.add_argument('--organs', nargs='*', default=DEFAULT_UBERON, help=f'UBERON terms. Default={DEFAULT_UBERON}.')
    ...
    P.add_argument(
        '--merge-distance', dest='merge_distance', type=int, default=300, help='Max bp gap to merge regions.'
    )
    P.add_argument('--window-size', dest='window_size', type=int, default=100, help='Sliding window size (bp).')
    P.add_argument(
        '--scan-span', dest='scan_span', type=int, default=50000, help='Bp each side of variant center to scan.'
    )
    P.add_argument('--plot-non-sig', dest='plot_non_sig', action='store_true', help='Plot even when not significant.')
    P.add_argument(
        '--scan-all-tracks', dest='scan_all_tracks', action='store_true', help='Scan all tracks (recommended).'
    )
